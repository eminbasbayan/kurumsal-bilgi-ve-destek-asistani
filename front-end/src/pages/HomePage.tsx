...
import { OPEN_STATUSES, go } from "../app/navigation";
...
export function HomePage({ requests }: { requests: SupportRequest[] }) {
  const open = requests.filter((item) =>
    OPEN_STATUSES.includes(item.status),
  ).length;
  const waiting = requests.filter(
    (item) => item.status === "Kullanıcıdan Bilgi Bekleniyor",
  ).length;
  return (
    <>
      <PageHeader
      ...
      <div className="dashboard">
        <Card className="panel">
          ...
          <RequestTable items={requests.slice(0, 4)} />
        </Card>
